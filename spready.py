...
def spread(df_prices, sharpe_lookback, switch):
#   df_spread = pd.merge(df_prices, df_prices, left_index=True, right_index=True, how='outer') 
    frame = []
    for i in range(1, 1+len(ls_symbols)):
	frame.append(df_prices)

    df_spread = pd.concat(frame,keys = ls_symbols)
#   df_spread = np.NAN * copy.deepcopy(df_spread)
    df_capm_gap = np.NAN * copy.deepcopy(df_prices)
    ldt_timestamps = df_capm_gap.index
    print df_spread
#   check    
#    for s1 in ls_symbols[:len(ls_symbols)]:
#	for s2 in ls_symbols[:len(ls_symbols)]:
#		for i in range(1 , len(ldt_timestamps)):  
#			print df_spread[s2].ix[[(s1,df_prices.index[i])]]	
#   returnization
    ts_index = df_prices[ls_symbols[-1]]
#   tsu.returnize0(ts_index)

    for s_sym in ls_symbols[:len(ls_symbols)]:
	tsu.returnize0(df_prices[s_sym])

# random check   
    print df_spread.ix[[('AMZN', '2013-11-14 16:00:00')]] 
    print df_spread.ix[[('HAIN', '2013-11-14 16:00:00')]]

    for x_symbol in ls_symbols[:len(ls_symbols)]:
        tx_price = df_prices[x_symbol]
#	print "x symbol is", x_symbol
#       print "returns", ts_price
#       print "index", ts_index
        ts_x_ret = pd.rolling_sum(tx_price, sharpe_lookback)
	
	for y_symbol in ls_symbols[:len(ls_symbols)]:
		ty_price = df_prices[y_symbol]
#		print ty_price
#		print "y symbol is", y_symbol, ty_price
        	ts_y_ret = pd.rolling_sum(ty_price, sharpe_lookback)

	        beta = (1/pd.rolling_var(tx_price, sharpe_lookback)) * pd.rolling_cov(tx_price, ty_price, sharpe_lookback)
        	alpha = pd.rolling_mean(ty_price, sharpe_lookback) - beta * pd.rolling_mean(tx_price, sharpe_lookback)

#		print x_symbol, "ret_x:", ts_x_ret, "ret_y:", ts_y_ret, y_symbol, "alpha:", alpha, "beta:",beta
                df_capm_gap[y_symbol]= ts_y_ret - alpha - beta * ts_x_ret
		for i in range(1 + sharpe_lookback, len(ldt_timestamps)):
			df_spread[y_symbol].ix[[(x_symbol,df_prices.index[i])]]=df_capm_gap[y_symbol].ix[df_prices.index[i]]
			print x_symbol, y_symbol, ldt_timestamps[i],df_spread[y_symbol].ix[[(x_symbol,df_prices.index[i])]], df_capm_gap[y_symbol].ix[df_prices.index[i]]
#	df_capm_gap = pd.concat([df_capm_gap, df_capm_gap])
# beta.ix[ldt_timestamps[len(ldt_timestamps)]], "alpha:", alpha.ix[ldt_timestamps[len(ldt_timestamps)]]
#       try PCA !
#       print "ind", ts_x_ret, "y", ts_y_ret, "a" , alpha, "b", beta, df_capm_gap[s_symbol]
#		df_spread[y_symbol].ix[[(x_symbol,)]]=alpha
#		ldt_timestamps = df_capm_gap.index
#    		for i in range(1 + sharpe_lookback, len(ldt_timestamps)):
#       df_capm_gap.ix[ldt_timestamps[i]]=scipy.stats.stats.rankdata(df_capm_gap.ix[ldt_timestamps[i]])
#			df_spread[y_symbol].ix[[(x_symbol,df_prices.index[i])]]=(ts_y_ret - alpha - beta * ts_x_ret).ix[ldt_timestamps[i]]
#			print "row:", x_symbol, y_symbol, ldt_timestamps[i],(ts_y_ret - alpha - beta * ts_x_ret).ix[ldt_timestamps[i]] 
#                	print "spread:", df_spread[y_symbol].ix[[(x_symbol,df_prices.index[i])]]   
#     	       df_spread.ix[[('AMZN',df_prices.index[i])]]
#	print "end lopp", x_symbol, y_symbol
    return df_spread
...
#   df_spread = pd.concat(frames, keys=ls_symbols)
    ...
    if (style==101):
	spread = spread(d_data["actual_close"], sharpe_lookback, switch2)
        save_sharpe(spread, s_sharpe_file_path)
#	print spread
        ldt_timestamps =  d_data["actual_close"].index
	for s1 in ls_symbols[:len(ls_symbols)]:
        	for s2 in ls_symbols[:len(ls_symbols)]:
        		print s1, s2, df_spread[s2].ix[[(s1,df_prices.index[len(ldt_timestamps)])]]        
#	save_sharpe(spread, s_sharpe_file_path)
