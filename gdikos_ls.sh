...
then sudo python ls.py $i $holding_period $trigger $market $bin $switch $style $sharpe_period $quantile $COUNTER >> sims/log_gdikos_12091607.txt && python ls_sim.py && python ls_analyzer.py $i $holding_period $trigger $market $bin $switch $style $sharpe_period $quantile $COUNTER >> sims/alpha_gdikos_12091607.txt
fi
COUNTER=$((COUNTER+1))
echo $COUNTER
done
